...
def geom_from_comb(trin_comb):
    ...
    return triangulation.Triangulation(tris_geom, gluings(trin_comb), K)
...
def get_idr(trin):
    return trin.geom.get_idr


def gen_idrs_nontrivial():
    idrs = [get_idr(trin) for trin in gen_trins()]
    return [IDR for IDR in idrs if IDR.is_nontrivial]


def gen_trins_with_idr_nontrivial():
    return [trin for trin in gen_trins() if get_idr(trin).is_nontrivial]
...
if __name__ == "__main__":
    G = algo.generators_veech(gen_trins_with_idr_nontrivial()[0].geom)
    print(G)
